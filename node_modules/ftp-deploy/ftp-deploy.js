...
var FtpDeployer = function () {
	// The varructor for the super class.
	...
	function ftpPut(partialFilePath, cb) {
		...
		var emitData = {
			...
			percentCompvare: Math.round((transferredFileCount / partialFilePaths.length) * 100),
  ...
  // A method for changing the remote working directory and creating one if it doesn't already exist
	...
	function configCompvare(config, cb) {
    // Init
		ftp = new Ftp({
			host: config.host,
			port: config.port
